...
$empleados = $empleado->usr_emp($_SESSION['idusuario']);
	if ($_SESSION['idperfil_anat']==1 or $_SESSION['idperfil_anat']==2) {
		...
		$biocv = new biopsia();
		$bioscv = $biocv->bioregCE();
		require 'view/CE/biopsiaCE-informacion.php';
	}
...
function _actualizarAction(){
    ...
    $biopsiap->obtenerinfoce();
	$topo=new Topografia();
    $topografias=$topo->topxce();
    $dep = new Dependencia();
    $deps=$dep->consultar();
    ...
    $tecs=$biop->listaproc();
    $biom=$biop->listamuece();
    

    require 'view/CE/bioCE-update.php';
}
