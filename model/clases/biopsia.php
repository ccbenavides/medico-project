<?php
 ...
 class Biopsia{
    ...
    function __construct($id_biopsia="",$id_paciente="",$num_biopsia="",$dep_id="",$medico_tratante="",$fecha_biopsia="",
                         ...
                         $diag_inicial="",$observacion="",$pago_paciente="",$id_usuario="",$estado_biopsia="",$codigo="",$tecnologo_responsable="",$id_anio="",$id_proc="",$medico_opcional=""){
    ...
    public function insertar(){
        $stmt=$this->objPDO->prepare('INSERT INTO sisanatom.biopsia (id_paciente,num_biopsia,id_institucion,dep_id,medico_tratante,fecha_biopsia,fecha_ingreso,
                             id_top,bio_congelacion,patologo_responsable,diag_inicial,observacion,pago_paciente,id_usuario,estado_biopsia,tipo_estudio,codigo,id_anio,id_proc,medico_opcional) 
    ...
    public function insertarbio(){
      $stmt=$this->objPDO->prepare('INSERT INTO sisanatom.biopsia (id_paciente,num_biopsia,id_institucion,dep_id,medico_tratante,fecha_biopsia,fecha_ingreso,
                             id_top,patologo_responsable,diag_inicial,observacion,pago_paciente,id_usuario,estado_biopsia,codigo,id_anio,id_proc,medico_opcional) 
                            VALUES(:id_paciente,:num_biopsia,:id_institucion,:dep_id,:medico_tratante,:fecha_biopsia,:fecha_ingreso,
                                :id_top,:patologo_responsable,:diag_inicial,:observacion,:pago_paciente,:id_usuario,:estado_biopsia,:codigo,:id_anio,:id_proc,:medico_opcional)');
        $stmt->execute(array('id_paciente'=>$this->id_paciente,
                             'num_biopsia'=>$this->num_biopsia,
   ...
   /*buscar si existe codigo de biopsia antes de insertarlo*/
    public function consultnumbiopsia($num_biopsia,$area){
        $stmt=$this->objPDO->prepare("SELECT * from sisanatom.biopsia b inner join sisanatom.topografia_area ta on b.id_top=ta.id_top
                                    inner join sisanatom.area a on ta.id_area=a.id_area where num_biopsia=:num_biopsia AND a.id_area=:area");
        $stmt->execute(array('num_biopsia' =>$num_biopsia,'area'=>$area));
        $cnumbio=$stmt->rowCount();
        return $cnumbio;
    }
    ...
    //informacion de biopsia para autorizar edicion
      public function bioxfechar($id_area,$fecha){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre) as paciente,
                                    ...
                                    on b.id_top=ta.id_top inner join sisanatom.area a on ta.id_area=a.id_area inner join sisemer.paciente pac on b.id_paciente=pac.id_paciente
    ...
    public function bioxfechar1($fecha){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre) as paciente,
                                    ...
                                    inner join sisanatom.detalle_bioccv cb on cb.id_biopsia=b.id_biopsia inner join sisemer.paciente pac on b.id_paciente=pac.id_paciente
                                    inner join empleados emp2 on b.patologo_responsable=emp2.emp_id
    ...
    public function listobio(){
        $stmt = $this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(p.a_paterno||' '||p.a_materno||' '||p.nombre)as paciente,a.id_area,b.fecha_ingreso,
                                        (CASE WHEN b.condicion_biopsia='A' THEN 'Activo' ELSE 'Inactivo' END)as estado_biopsia from sisanatom.biopsia b 
                                        inner join sisemer.paciente p on b.id_paciente=p.id_paciente inner join sisanatom.topografia_area ta on b.id_top=ta.id_top inner join sisanatom.area a on a.id_area=ta.id_area
                                        order by a.id_area,b.fecha_ingreso ASC ");
        $stmt->execute();
    ...
    public function actualizabaja($id_biopsia){
       ...
       $sql = "UPDATE sisanatom.biopsia set condicion_biopsia='I'
     ...
     public function actualizadetallemovbio($id_biopsia,$descr_mov,$motivo,$id_usuregmov){
        
        $stmt =$this->objPDO->prepare("INSERT INTO sisanatom.mov_bio(id_biopsia,descr_mov,motivo,id_usuregmov)
                                       values(:id_biopsia,:descr_mov,:motivo,:id_usuregmov);");
        $stmt->execute(array('id_biopsia' => $this->id_biopsia,
                             'descr_mov'=>$this->descr_mov,
                             'motivo'=>$this->motivo,
                             'id_usuregmov'=>$this->id_usuregmov));
    }

    public function actualizaalta($id_biopsia){
       $sql = "UPDATE sisanatom.biopsia set condicion_biopsia='A'
                where id_biopsia=:id_biopsia;";
...
/*codigo por cada area*/
    ...
    //obtener el maximo codigo de cada area para colocarlo en la etiqueta
    public function obtmaxcodarea($id_area){
        $stmt=$this->objPDO->prepare("SELECT ltrim(to_char(max(b.codigo),'00000'))as codigo from sisanatom.biopsia b inner join sisanatom.topografia_area ta on b.id_top=ta.id_top
                                    inner join sisanatom.area a on ta.id_area=a.id_area
                                    where a.id_area=:id_area and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (select current_date))");
        $stmt->execute(array('id_area'=>$id_area));
        $codxar=$stmt->fetchAll(PDO::FETCH_OBJ);
        return $codxar[0]->codigo;
    }

    //obtener el minimo codigo de cada area para colocarlo en la etiqueta
    public function obtminxcodarea($id_area){
        $stmt=$this->objPDO->prepare("SELECT ltrim(to_char(min(b.codigo),'00000'))as codigo from sisanatom.biopsia b inner join sisanatom.topografia_area ta on b.id_top=ta.id_top
                                    inner join sisanatom.area a on ta.id_area=a.id_area
                                    where a.id_area=:id_area and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (select current_date))");
        $stmt->execute(array('id_area'=>$id_area));
        $codxar=$stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    public function codigoce(){
        $stmt=$this->objPDO->prepare("SELECT ltrim(to_char((CASE WHEN max(codigo)is null THEN 1 ELSE max(codigo)+1 END),'00000')) as nuevo from sisanatom.biopsia b
                                      inner join sisanatom.topografia_area ta on ta.id_top=b.id_top
                                      WHERE EXTRACT(YEAR FROM fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date)) and ta.id_area=2 or ta.id_area=3");
        $stmt->execute();
    ...
    public function codigoccv(){
        $stmt=$this->objPDO->prepare("SELECT ltrim(to_char((CASE WHEN max(codigo)is null THEN 1 ELSE max(codigo)+1 END),'00000')) as nuevo from sisanatom.biopsia b
                                      inner join sisanatom.topografia_area ta on ta.id_top=b.id_top
                                      WHERE EXTRACT(YEAR FROM fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date)) and ta.id_area=3 or ta.id_area=2");
        $stmt->execute();
   ...
   public function codnumbio($num_biopsia){
    ...
    public function codcanastilla($id_area,$codigo1,$codigo2){
        $stmt=$this->objPDO->prepare("SELECT b.num_biopsia,b.num_biopsia,b.num_biopsia,b.num_biopsia,b.num_biopsia from sisanatom.biopsia b inner join sisanatom.topografia_area a on b.id_top=a.id_top
                                      where a.id_area=:id_area and (b.codigo>=:codigo1 and b.codigo<=:codigo2) and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (select current_date))
...
/*LISTADO DE BIOPSIAS ASIGNADAS A TODOS LOS PATOLOGOS EN DIFERENTES AREAS*/
    ...
    //listar biopsias de PQ para los tecnicos de recepcion
    public function bioregPQ(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre)as paciente,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        list(m.descr_muestra)as muestras from sisanatom.biopsia b 
                        inner join sisanatom.detalle_biopq db on b.id_biopsia=db.id_biopsia
                        inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                        inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem inner join sisemer.paciente pac on b.id_paciente=pac.id_paciente
                        where b.estado_biopsia>=1 and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (SELECT current_date)) and b.condicion_biopsia='A'
                        group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico,pac.a_paterno,pac.a_materno,pac.nombre  order by b.id_biopsia desc");
        $stmt->execute();
        $biorspq=$stmt->fetchAll(PDO::FETCH_OBJ);
        return $biorspq;
    }
    ...
    public function biotecPQ(){
      $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        ...
                        inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                        inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                        where b.estado_biopsia<2 and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (SELECT current_date)) and b.condicion_biopsia='A'
    ...
    public function biotecnIH(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        ...
                        inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                        inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                        where b.estado_biopsia<2 and b.condicion_biopsia='A'
                        group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
    $stmt->execute();
    ...
    //listar biopsias registradas de CE para tecnicos de recepcion
    public function bioregCE(){
       $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre)as paciente,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        list(m.descr_muestra)as muestras,b.estado_biopsia from sisanatom.biopsia b 
                        inner join sisanatom.detalle_bioce db on b.id_biopsia=db.id_biopsia
                        inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                        inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem inner join sisemer.paciente pac on b.id_paciente=pac.id_paciente
                        where b.estado_biopsia>=1 and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (SELECT current_date)) and b.condicion_biopsia='A'
                        group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico,pac.a_paterno,pac.a_materno,pac.nombre  order by b.id_biopsia desc");
        $stmt->execute();
        $biorsce=$stmt->fetchAll(PDO::FETCH_OBJ);
        return $biorsce; 
    }
    ...
    //listar biopsias registradas de CCV para tecnicos de recepcion
    public function bioregCCV(){
      $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre)as paciente,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        list(m.descr_muestra)as muestras from sisanatom.biopsia b 
                        ...
                        group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico,pac.a_paterno,pac.a_materno,pac.nombre  order by b.id_biopsia desc");
    $stmt->execute();
    $biosrccv=$stmt->fetchAll(PDO::FETCH_OBJ);
    return $biosrccv;  
    }
    ...
    //listar biopsias registradas de IH para tecnicos de recepcion
    public function bioregIH(){
       $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(pac.a_paterno||' '||pac.a_materno||' '||pac.nombre)as paciente,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,
                        list(m.descr_muestra)as muestras from sisanatom.biopsia b 
                        inner join sisanatom.detalle_bioih db on b.id_biopsia=db.id_biopsia
                        inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                        ...
                        group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico,pac.a_paterno,pac.a_materno,pac.nombre  order by b.id_biopsia desc");
        $stmt->execute();
        $biorsih=$stmt->fetchAll(PDO::FETCH_OBJ);
        return $biorsih; 
    }
/*LISTADO DE BIOPSIAS ASIGNADAS POR PATOLOGO */
    ...
    public function biopqxpat($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,
                            ...
                            where e.emp_id=:emp_id and b.estado_biopsia<3 and EXTRACT(YEAR FROM b.fecha_ingreso)=EXTRACT(YEAR FROM (SELECT current_date)) and b.condicion_biopsia='A'
                            group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
    ...
    public function bioccvxpat($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,
                            ...
                            inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                            inner join sisanatom.usuarios_anat ua on e.emp_id=ua.emp_id inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                            where e.emp_id=:emp_id and b.estado_biopsia<3  and b.condicion_biopsia='A'
                            group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute(array('emp_id'=>$emp_id));
    ...
    public function biopqxtec($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,
                            ...
                            inner join sisanatom.usuarios_anat ua on e.emp_id=ua.emp_id inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                            where e.emp_id=:emp_id and b.estado_biopsia<2 and EXTRACT(YEAR FROM b.fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date)) and b.condicion_biopsia='A'
                            group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico
                            order by codigo asc ");
        $stmt->execute(array('emp_id'=>$emp_id));
...
/*LISTADO DE BIOPSIAS PENDIENTES DE PATOLOGIA QUIRUGICA E INMUNOHISTOQUIMICA CON ALARMA DE 10 DIAS*/
    ...
    public function biopendpq(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=10 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=10 and EXTRACT(YEAR FROM b.fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date))
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute();
    ...
    public function biopqxemp($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=10 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=10 and b.patologo_responsable=:emp_id and EXTRACT(YEAR FROM b.fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date))
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute(array('emp_id'=>$emp_id));
    ...
    public function biopendih(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=10 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                            ...
                            inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                            inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                            where b.estado_biopsia<3 and (date_part('day',((select now())-b.fecha_ingreso)))>=10 and b.condicion_biopsia='A'
                            group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute();
    ...
    public function bioihporemp($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=10 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                            ...
                            inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                            inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                            where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=10 and b.patologo_responsable=:emp_id 
                            group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute(array('emp_id'=>$emp_id));
...
/*LISTADO DE BIOPSIAS PENDIENTES DE CITOLOGIA ESPECIAL Y CERVICO VAGINAL CON ALARMA DE 7 DIAS*/
    ...
    public function biopendce(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=7 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                    ...
                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=7 and EXTRACT(YEAR FROM b.fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date))
                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute();
    ...
    public function biocexemp($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=7 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                    ...
                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=7 and b.patologo_responsable=:emp_id and EXTRACT(YEAR FROM b.fecha_ingreso)=(SELECT EXTRACT(YEAR FROM current_date))
                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute(array('emp_id'=>$emp_id));
    ...
    public function biopendccv(){
     ...
     public function bioccvxemp($emp_id){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=7 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                ...
                                inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                                inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=7 and b.patologo_responsable=:emp_id
                                group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute(array('emp_id'=>$emp_id));
...
/*LISTADO DE BIOPSIAS CON TACOS PENDIENTES DE TODAS LAS AREAS CON UNA ALARMA DE IGUAL O MAYOR A 6 DIAS*/
    ...
    public function tacpqporemp($tecnologo){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))>=6 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.tecnologo_responsable=e.emp_id
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    where db.num_tacos=0 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))>=6 and b.tecnologo_responsable=:tecnologo
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,tecnologo ");
    ...
    public function biopqporserpendiente(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))=9 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))=9
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute();
    ...
    public function bioceporserpendiente(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))=6 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestras_biopsia mb on b.id_biopsia=mb.id_biopsia inner join empleados e on b.patologo_responsable=e.emp_id
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))=6
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute();
    ...
    public function bioxtecpqspend($id_usuario){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))=5 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    where db.num_tacos=0 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))=5 and ua.id_usuario=:id_usuario and b.estado_biopsia<2
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,tecnologo");
    ...
    public function bioxemppqspend($id_usuario){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))=9 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))=9 and ua.id_usuario=:id_usuario
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico ");
        $stmt->execute(array('id_usuario'=>$id_usuario));
    ...
    public function bioxempcespend($id_usuario){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(case when (date_part('day',((select now())-b.fecha_ingreso)))=6 then date_part('day',((select now())-b.fecha_ingreso)) end) 
                                    ...
                                    inner join sisanatom.muestra m on m.id_muestra=mb.id_muestrarem
                                    inner join sisanatom.usuarios_anat ua on ua.emp_id=e.emp_id
                                    where b.estado_biopsia<3 and b.condicion_biopsia='A' and (date_part('day',((select now())-b.fecha_ingreso)))=6 and ua.id_usuario=:id_usuario
                                    group by b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico");
        $stmt->execute(array('id_usuario'=>$id_usuario));
    ...
    public function alertaparaadmin(){
        $stmt=$this->objPDO->prepare("SELECT listapq.id_biopsia,listapq.num_biopsia,(CASE WHEN (date_part('day',((select now())-listapq.fecha_ingreso)))>=muepq.min then
                                    ...
                                    WHERE date_part('day',((select now())-listapq.fecha_ingreso))>=muepq.min and EXTRACT(YEAR FROM listapq.fecha_ingreso)=EXTRACT(YEAR FROM (select current_date)) ORDER BY listapq.fecha_ingreso DESC ");
        $stmt->execute();
    ...
    public function alertaparatec($id_usuario){
        $stmt=$this->objPDO->prepare("SELECT listapq.id_biopsia,listapq.num_biopsia,(CASE WHEN (date_part('day',((select now())-listapq.fecha_ingreso)))>=muepq.min then
                                    ...
                                    (SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,list(m.descr_muestra)as muestras,a.id_area from sisanatom.biopsia b 
                                                                        ...
                                                                        inner join sisanatom.topografia_area ta on b.id_top=ta.id_top inner join sisanatom.area a on a.id_area=ta.id_area
                                                        inner join sisanatom.usuarios_anat ua on ua.emp_id=e.emp_id
                                                                        where b.estado_biopsia<2 and b.condicion_biopsia='A' and ua.id_usuario=:id_usuario
                                                                        group by a.id_area,b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico)listapq inner join
                                    (select mb.id_biopsia,min(m.tiempo_estudio) from sisanatom.muestras_biopsia mb inner join sisanatom.muestra m on mb.id_muestrarem=m.id_muestra 
                                    inner join sisanatom.biopsia b on mb.id_biopsia=b.id_biopsia 
                                    group by mb.id_biopsia)muepq on listapq.id_biopsia=muepq.id_biopsia
    ...
    public function alertapat($id_usuario){
        $stmt=$this->objPDO->prepare("SELECT listapq.id_biopsia,listapq.num_biopsia,(CASE WHEN (date_part('day',((select now())-listapq.fecha_ingreso)))>=muepq.min then
                                        ...
                                        (SELECT b.id_biopsia,b.num_biopsia,b.fecha_ingreso,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno) as medico,list(m.descr_muestra)as muestras,a.id_area from sisanatom.biopsia b 
                                                                            ...
                                                                            inner join sisanatom.topografia_area ta on b.id_top=ta.id_top inner join sisanatom.area a on a.id_area=ta.id_area
                                                            inner join sisanatom.usuarios_anat ua on ua.emp_id=e.emp_id
                                                                            where b.estado_biopsia<3 and b.condicion_biopsia='A' and ua.id_usuario=:id_usuario
                                                                            group by a.id_area,b.id_biopsia,b.num_biopsia,b.fecha_ingreso,medico)listapq inner join
    ...
    public function consultarxpac($num_biopsia,$area){
        $stmt = $this->objPDO->prepare("SELECT * from sisanatom.biopsia b inner join sisanatom.topografia_area ta on b.id_top=ta.id_top
                                        inner join sisanatom.area a on ta.id_area=a.id_area
                                        where num_biopsia=:num_biopsia AND a.id_area=:area");
        $stmt->execute(array('num_biopsia' => $num_biopsia,'area'=>$area));
    ...
    public function consultnumbio($num_biopsia,$id_area){
        $stmt=$this->objPDO->prepare("SELECT * from sisanatom.biopsia b inner join sisanatom.topografia_area ta on b.id_top=ta.id_top
                                      inner join sisanatom.area a on ta.id_area=a.id_area 
                                      where b.num_biopsia=:num_biopsia AND a.id_area=:id_area");
        $stmt->execute(array('num_biopsia' =>$num_biopsia,'id_area'=>$id_area));
    ...
    public function listatecpat(){
        $stmt=$this->objPDO->prepare("SELECT b.id_biopsia,b.num_biopsia,(e.emp_nombres||' '||e.emp_appaterno||' '||e.emp_apmaterno)as patologo,(ep.emp_nombres||' '||ep.emp_appaterno||' '||ep.emp_apmaterno) as tecnologo,
                                    ...
                                    where b.id_top!=25 and b.condicion_biopsia='A'
    ...
    public function setProc($id_proc){
        $this->id_proc=$id_proc;
    }

    public function getMopcional(){
        return $this->medico_opcional;
    }
    public function setMopcional($medico_opcional){
        $this->medico_opcional=$medico_opcional;
    }

    public function getdescrmov(){
        return $this->descr_mov;
    }
    public function setdescrmov($descr_mov){
        $this->descr_mov=$descr_mov;
    }
    ...
    public function getusuregmov(){
        return $this->id_usuregmov;
    }
    public function setusuregmov($id_usuregmov){
        $this->id_usuregmov=$id_usuregmov;
    }
