...
if ($fecha1!=$fecha2) {
	...
	$pdf->Cell(0,5,"ESTADISTICAS DE BIOPSIAS POR CONGELACION DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio." AL ".$fecha2_dia."-".$fecha2_mes."-".$fecha2_anio,0,0,'C');
	$titu="ESTADISTICAS DE BIOPSIAS POR CONGELACION DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio." AL ".$fecha2_dia."-".$fecha2_mes."-".$fecha2_anio;
} else {
	$pdf->SetXY(10,30);
	$pdf->SetFont('Arial','B',10.5);
	$pdf->Cell(0,5,"ESTADISTICAS DE BIOPSIAS POR CONGELACION DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio,0,0,'C');
	$titu="ESTADISTICAS DE BIOPSIAS POR CONGELACION DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio;
}
$pdf->Ln(10);
$pdf->SetX(60);
$pdf->SetFont('Arial','B',8.5);
$pdf->Cell(55,5,utf8_decode("BIOPSIA POR CONGELACION"),1,0,'C');
$pdf->Cell(30,5,utf8_decode("CANTIDAD"),1,0,'C');
$pdf->Ln(5);
$totales=0;
for ($i=0; $i < count($biocongela); $i++) { 
	$pdf->SetX(60);
	$pdf->SetFont('Arial','',8.5);
	$pdf->Cell(55,5,$biocongela[$i]->estado,1,0,'C');
	$pdf->Cell(30,5,$biocongela[$i]->total,1,0,'C');
	$pdf->Ln(5);
	$totales=$totales+$biocongela[$i]->total;
}
...
$pdf->Ln(10);
if ($totales!=0) {
	$pdf->graficosestadistico($ntot,$titu,$congelacion,"graficos",25);
}
$pdf->Output();
