...
class PDF extends FPDF{
	...
	public function grafico01($datos1=array(),$meses=array(),$titulo=NULL,$nomgrafico=NULL,$x){
	    ...
	    $graph->xaxis->title->Set('SERVICIO HRL');
...
if ($fecha1!=$fecha2) {
	...
	$pdf->Cell(0,5,"ESTADISTICAS DE BIOPSIAS REALIZADAS POR SERVICIO DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio." AL ".$fecha2_dia."-".$fecha2_mes."-".$fecha2_anio,0,0,'C');
	$titu="ESTADISTICAS DE BIOPSIAS REALIZADAS POR SERVICIO DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio." AL ".$fecha2_dia."-".$fecha2_mes."-".$fecha2_anio;
} else {
	$pdf->SetXY(10,30);
	$pdf->SetFont('Arial','B',10.5);
	$pdf->Cell(0,5,"ESTADISTICAS DE BIOPSIAS POR SERVICIO DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio,0,0,'C');
	$titu="ESTADISTICAS DE BIOPSIAS POR SERVICIO DEL ".$fecha1_dia."-".$fecha1_mes."-".$fecha1_anio;
}
...
$pdf->Cell(90,5,utf8_decode("SERVICIO HRL"),1,0,'C');
...
for ($i=0; $i <count($servicios) ; $i++) { 
	...
	$pdf->Cell(15,5,$servicios[$i]->dep_id,1,0,'C');
	$pdf->Cell(90,5,utf8_decode($servicios[$i]->dep_descr),1,0,'L');
	$pdf->Cell(55,5,$servicios[$i]->total_bio,1,0,'C');
	$pdf->Ln(5);
	$ct=$ct+$servicios[$i]->total_bio;
}
...
$pdf->SetX(2);
if ($ct!=0) {
	$pdf->grafico01($cant,$dependencias,$titu,"graficos",10);
}
