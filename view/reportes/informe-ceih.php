...
if ($cantidad>0) {
	...
	$pdf->Cell(45,5,utf8_decode("Inmunohistoquimica"),0,0,'C');
	$pdf->Cell(136,5,utf8_decode("Inmunohistoquimica"),0,0,'R');
	$pdf->Ln(5);
	$pdf->tablewidths=array(7,123,20,123);
	for ($n=0; $n <count($marcadoresCE) ; $n++) { 	
	$pdf->SetFont('Arial', '',8.5);
	$marc[]=array('',($n+1).". ".utf8_decode($marcadoresCE[$n]->descr_marcador)." : ".utf8_decode($marcadoresCE[$n]->resultado),'',($n+1).". ".utf8_decode($marcadoresCE[$n]->descr_marcador)." : ".utf8_decode($marcadoresCE[$n]->resultado));
	}
	$pdf->morepagestable($marc);
	$pdf->Ln(3);
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(41,5,chr(149).utf8_decode(" MACROSCOPIA : "),0,0,'C');
	$pdf->Cell(139,5,chr(149).utf8_decode(" MACROSCOPIA : "),0,0,'R');
	$pdf->Ln(5);
	$pdf->tablewidths=array(7,123,20,123);
	for ($n=0; $n < count($macrosCE) ; $n++) { 
		$pdf->SetFont('Arial', '',7.5);
		$filas[]=array('',chr(149).' '.strtoupper(utf8_decode($macrosCE[$n]->macroscopia)),'',chr(149).' '.strtoupper(utf8_decode($macrosCE[$n]->macroscopia)));
	}
	...
	$pdf->Ln(2.5);
	$pdf->tablewidths=array(58,100,43,100);
	...
	for ($j=0; $j < count($biopsiasCE) ; $j++) { 
		$pdf->SetFont('Arial', 'B',8);
		$colegiaturas[]=array('',''.chr(32).chr(32).chr(32).chr(32).utf8_decode("C.M.P."." ".$biopsiasCE[$j]->emp_colegiatura),'',utf8_decode("C.M.P."." ".$biopsiasCE[$j]->emp_colegiatura));
	}
