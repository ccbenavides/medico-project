	...
	function Header()
	  {
	    ...
	    $this->Cell(190,2, "HOSPITALl REGIONAL LAMBAYEQUE", 0, 0, 'C');
	    $this->Ln(7);
...
if ($biopsiasPQ) {
	foreach ($biopsiasPQ as $key) {
		...
		$pdf->SetFont('Arial', '',8.5);
		$pdf->Cell(0,58,$key->medico_tratante,0,0,'L');
		// $pdf->SetFont('Arial', '',9);
...
if ($cantidad>0) {
	...
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(58,5,"Inmunohistoquimica:",0,0,'C');
	$pdf->Ln(7);
	for ($i=0; $i <count($marcadoresPQ) ; $i++) { 
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(15,5,($i+1).".",0,0,'R');
		$pdf->Cell(80,5,utf8_decode($marcadoresPQ[$i]->descr_marcador." : ".$marcadoresPQ[$i]->resultado),0,0,'L');
		$pdf->Ln(5);
	}$pdf->Ln(1);
	$pdf->SetFont('Arial','B',10);
	...
	if ($macroPQ) {
		foreach ($macroPQ as $macro) {
			...
			$pdf->Cell(10,5,'',0,0,'C');
			$pdf->MultiCell(180,5,strtoupper(utf8_decode($macro->macroscopia)),0,'J');
		}
	}$pdf->Ln(15);
	$pdf->Cell(268,5,"----------------------------------------------------------------------------------------------",0,0,'C');
	$pdf->Ln(5);
	if ($biopsiasPQ) {
		foreach ($biopsiasPQ as $key) {
			...
			$pdf->Ln(5);
			// $pdf->Cell(145,5,"C.M.P."." ".$key->emp_colegiatura,0,0,'R');
			$pdf->Cell(268,5,"C.M.P."." ".$key->emp_colegiatura.chr(32).chr(32).chr(32)."R.N.E"." ".$key->emp_rne,0,0,'C');
		}
	}
}
$pdf->Output();
